...
export const AddProductToCart_a11yAssert = base.extend<{ AddProductToCart_a11yAssert: Task }, FixtureTypes>({
    AddProductToCart_a11yAssert: async ({ ShopCustomer, StorefrontProductDetail }, use) => {
        const task = (ProductData: Product, quantity = '1') => {
            return async function AddProductToCart() {
                ...
                await StorefrontProductDetail.addToCartButton.press('Enter');
                */

                await ShopCustomer.fillsIn(StorefrontProductDetail.quantitySelect, quantity);
                await ShopCustomer.presses(StorefrontProductDetail.addToCartButton, 'Enter');
