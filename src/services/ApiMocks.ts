...
export async function mockApiCalls(page: Page | BrowserContext) {
    ...
    await page.route("**/api/_action/store/plugin/search*", (route) =>
        route.fulfill({
            status: 200,
            contentType: "application/json",
    ...
    await page.route("**/api/_action/store/updates*", (route) =>
        route.fulfill({
            status: 200,
            contentType: "application/json",
            body: JSON.stringify({ items: [], total: 0 }),
    ...
    await page.route("**/api/sbp/shop-info*", (route) =>
        route.fulfill({
            status: 200,
            contentType: "application/json",
            body: JSON.stringify({ items: [], total: 0 }),
    ...
    await page.route("**/api/sbp/bookableplans*", (route) =>
        route.fulfill({
            status: 200,
            contentType: "application/json",
    ...
    await page.route("**/api/sbp/nps/active-trigger", (route) =>
        route.fulfill({
            status: 200,
            contentType: "application/json",
