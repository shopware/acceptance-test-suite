...
test("Order creation with TestDataService", async ({ TestDataService, AdminApiContext }) => {
    ...
    const promotionWithCode = await TestDataService.createPromotionWithCode({
        code: "code1234",
        discounts: [{ scope: "cart", type: "absolute", value: 10, considerAdvancedRules: false }],
    });
    ...
    const orderWithCustomShippingCosts = await TestDataService.createOrder([{ product, quantity: 5 }], customer, customShippingCosts);
