...
The type definitions list the real exports, fixtures, and task names for the installed version. Task argument shapes are not there: `Task` is typed `(...args: any[]) => () => Promise<void>`, so read a task's real parameters in `dist/index.mjs`, and do not expect `tsc` to catch wrong task arguments. Published examples elsewhere sometimes use fixture names from older majors; prefer what the installed types show. If this skill and the installed package disagree, follow the package and tell the user exactly what differed: the skill's claim, the installed version, and what you did instead. If the skill looks genuinely stale rather than locally patched, suggest the user report it upstream (an issue or PR against shopware/acceptance-test-suite).
