...
process.env["MAILPIT_BASE_URL"] =
  process.env["MAILPIT_BASE_URL"] || "http://localhost:8013";
...
if (!process.env["WEBSERVER_COMMAND"]) {
  if (process.env["APP_URL"] === defaultAppUrl) {
    process.env["WEBSERVER_COMMAND"] =
      "docker compose up --pull=always --quiet-pull";
  } else {
...
const envLang =
  process.env.LANG || process.env.LANGUAGE || process.env.lang || "en-GB";
// Normalize locale: extract locale code and map to browser locale
const locale = envLang.split(".")[0].replace(/_/g, "-");
...
const browserLocale = supportedLocales[locale] || "en-DE";
